...
const getSellPriceByFacilityServiceId = async (facilityServiceId) => {
  ...
  const getMinMaxPriceOfHotel = async (districtId, privatetourRequestId, ratingId, pageNumber, pageSize) => {
    ...
    const getReferenceTransportByProvince = async (startPoint, endPoint) => {
      try {
        const response = await api.get(`/sell-price/get-reference-transport-by-province/${startPoint}/${endPoint}`);
        return response.data;
      } catch (error) {
